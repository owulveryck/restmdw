...
var staticContent = flag.String("staticPath", "./swagger-ui/dist", "Path to folder with Swagger UI")
...
func main() {
	...
	err := http.ListenAndServe(listenTo, http.DefaultServeMux)
	if err != nil {
		log.Println(err.Error())
	}
	log.Println("Server on %s stopped", listenTo)
	os.Exit(0)
}
